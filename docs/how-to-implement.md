...
2. **Using another router (Physical or virtualized)**: This is unconventional, but is better for people who don't have L3 switch or their network infrastructure is virtualized. You can use a router by disabling the WAN features (By plugging the firewall link into its LAN port, for example). If you are virtualizing, use a lightweight routing OS (Like OpenWRT) and configure it as such.
